                                  ...
                                  <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                                    ...
                                    {binding.dateOffset && (
                                      <>
                                        ...
                                        {(() => {
                                          const sKey = `${idx}-${ph}`;
                                          const result = computeDateOffsetPreview(binding.dateOffset, sampleDates[sKey]);
                                          return result
                                            ? <span style={{ color: "#16a34a", fontSize: "12px", fontWeight: 600 }}>→ {result}</span>
                                            : sampleDates[sKey] ? <span style={{ color: "#dc2626", fontSize: "11px" }}>Not a valid date — fallback used</span> : null;
                                        })()}
                                      </>
                                    )}
                                    {binding.dateOffset && (
                                      <input
                                        ...
                                        value={sampleDates[`${idx}-${ph}`] ?? ""}
                                        onChange={(e) => setSampleDates((s) => ({ ...s, [`${idx}-${ph}`]: e.target.value }))}
                                      />
                                    )}
                                  </div>
                                )}
