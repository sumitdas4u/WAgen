...
function buildWidgetScript(config: WidgetFormState, workspaceId: string, apiBase: string): string {
  const position = config.widgetPosition === "LEFT" ? "left" : "right";
  return `<script
  src="${escapeScriptString(apiBase)}/sdk/chatbot.bundle.js"
  wid="${escapeScriptString(workspaceId)}"
  data-theme-color="${escapeScriptString(config.brandColor)}"
  data-position="${position}"
  data-greeting="${escapeScriptString(config.welcomeMessage)}"
  data-api-base="${escapeScriptString(apiBase)}">
</script>`;
}
...
export function WidgetPage() {
  ...
  const canGenerate = Boolean(workspaceId) && isConfirmed;
  const scriptSnippet = useMemo(
    () => buildWidgetScript(form, workspaceId, apiBase),
    [apiBase, form, workspaceId]
  );
    ...
    <main className="widget-builder-page">
      <header className="widget-builder-topbar">
        <div>
          ...
          <p>Copy this code to your website to enable live AI chat and route messages into the unified inbox.</p>
        </div>
      ...
      <section className="widget-builder-layout">
        <section className="widget-builder-form-col">
          <article className="widget-form-card">
            <h2>Widget Channel Setup</h2>
            <div className="widget-form-grid">
              <label>
                Workspace ID
                <input value={workspaceId || "Unavailable"} readOnly />
              </label>
              <p className="tiny-note">
                Endpoint: <strong>{apiBase}/ws/widget</strong>. Website visitors will appear as <strong>Web</strong> channel chats.
              </p>
            </div>
          </article>
